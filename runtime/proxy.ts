...
interface Session {
    ...
    /** Set to the initError message when the worker module failed to load. */
    dead?: string;
    onDrained?: () => void;
}
...
export function createWorkerProxy(rawUrl: string): WorkerProxy {
    ...
    function spawn(): Session {
        const emitter = createWorkerEmitter<ParentIn, ParentOut>(
...
/**
 * Terminate the worker behind a `*.worker.ts` module.
 * Usage: `await closeWorker(import.meta.resolve("./math.worker.ts"))`.
 * Resolves immediately if no worker exists for the URL.
 */
...
export async function closeAllWorkers(opts?: CloseOptions): Promise<void> {
    await Promise.all([...registry.values()].map((p) => p.close(opts)));
}
